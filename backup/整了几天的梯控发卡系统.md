### 初衷 
虽说这样系统没什么研究价值了，但是我这里遇到一个差点就解不开卡了。好在有惊无险的解开了 
系统为互联创，没开启滚动位 ，一卡一密的系统 ，也可以初始滚动位。
问别问要了个密钥计算软件，好在还有个算法表 ，基于这个算法表 使用python 整出了软件，
### 奇怪的BUG 
虽然搞出来了，但是大部分还是算的对 ，只是有的特殊字节密钥会算错，搞不懂 勉强这样用吧！
还跟据算法添加了反推园区码功能。咱也没验证这个功能准确不准确，但是基于样本反推出来的基本是准确的，有带验证。
### 贴出代码作为备份！
后续如果算法完善了，后续基于本代码继续添加优化！
~~~
...
class ProfessionalCardToolV7:
    def __init__(self, root):
        self.root = root
        self.root.title("互联创密钥计算 - 三重碰撞深度分析版")
    ...
    def copy_to_clipboard(self, content, mode="KEY"):
        ...
        self.root.update()
        msg = f"成功：已复制密钥 [{content}]" if mode=="KEY" else f"成功：已复制园区码 [{content}]"
        self.lbl_status.config(text=msg, fg="#2ecc71", font=('微软雅黑', 9, 'bold'))

    def setup_ui(self):
        header = tk.Frame(self.root, bg="#2c3e50", height=60)
        header.pack(fill='x')
        tk.Label(header, text="互联创密钥算法分析系统", fg="white", bg="#2c3e50", font=('微软雅黑', 14, 'bold')).pack(pady=15)
        ...
        notebook.add(fwd_frame, text=" 🔒 密钥生成 ")
        
        self.create_input_section(fwd_frame, "8位原始卡号 (Card Hex):", "", "ent_card")
        self.create_input_section(fwd_frame, "8位园区代码 (Park Code):", "", "ent_park")

        btn_fwd = tk.Button(fwd_frame, text="生成并复制密钥 B", command=self.calc_forward, 
        ...
        notebook.add(rev_frame, text=" 🔓 逆向分析 (三重碰撞) ")

        # 样本 1
        s1_labelframe = tk.LabelFrame(rev_frame, text=" 数据样本 1 ", bg="white", fg="#2196F3")
        s1_labelframe.pack(fill='x', padx=20, pady=5)
        self.ent_rev_c1 = self.create_rev_entry(s1_labelframe, "卡号 1:")
        self.ent_rev_k1 = self.create_rev_entry(s1_labelframe, "密钥 1:")

        # 样本 2
        s2_labelframe = tk.LabelFrame(rev_frame, text=" 数据样本 2 ", bg="white", fg="#2196F3")
        s2_labelframe.pack(fill='x', padx=20, pady=5)
        self.ent_rev_c2 = self.create_rev_entry(s2_labelframe, "卡号 2:")
        self.ent_rev_k2 = self.create_rev_entry(s2_labelframe, "密钥 2:")

        # 样本 3
        s3_labelframe = tk.LabelFrame(rev_frame, text=" 数据样本 3 ", bg="white", fg="#2196F3")
        s3_labelframe.pack(fill='x', padx=20, pady=5)
        self.ent_rev_c3 = self.create_rev_entry(s3_labelframe, "卡号 3:")
        self.ent_rev_k3 = self.create_rev_entry(s3_labelframe, "密钥 3:")

        btn_rev = tk.Button(rev_frame, text="执行三重逻辑碰撞分析", command=self.calc_backward, 
                           bg="#e67e22", fg="white", font=('微软雅黑', 11, 'bold'), 
                           relief='flat', cursor='hand2', pady=10)
        ...
        self.lbl_status = tk.Label(self.root, text="系统就绪 | 多组输入可消除反推误差", bd=0, bg="#ecf0f1", fg="#95a5a6", anchor=tk.W, padx=10, pady=5)
    ...
    def create_input_section(self, parent, label_text, default_val, attr_name):
        ...
        tk.Label(frame, text=label_text, bg="white", fg="#34495e", font=('微软雅黑', 9, 'bold')).pack(anchor='w')
    ...
    def calc_forward(self):
        try:
            ...
            if len(c) != 8 or len(p) != 8: raise ValueError("请输入完整的 8 位十六进制数据")
            ...
            self.lbl_status.config(text=f"错误：{str(e)}", fg="#e74c3c")
    ...
    def calc_backward(self):
        try:
            ...
            if not active_samples: raise ValueError("请至少输入一组完整的卡号和密钥")
            ...
            def find_p_candidates(cb, kb, f_type, target):
                ...
                for p in range(256):
                    ...
                    if (v & 0xFF) == target:
                        p_str = f"{p:02X}"
                        if p_str.isdigit(): matches.add(p_str) # 锁定纯数字园区码
                return matches
            ...
            for byte_pos in range(4): # P1, P2, P3, P4
                ...
                if intersect:
                    # 如果有多个数字解，取数值最小的（通常只有一个解）
                    chosen = sorted(list(intersect))[0]
                    final_park_bytes.append(chosen)
                    report += f" P{byte_pos+1} 成功锁定: {chosen} (候选: {list(intersect)})\n"
                else:
                    # 容错处理
                    report += f" P{byte_pos+1} 碰撞失败! (交集为空)\n"
